...
int main() {
    ...
    int* dA, * dB, * dC;
    cudaMalloc(&dA, vectorBytes);
    cudaMalloc(&dB, vectorBytes);
    cudaMalloc(&dC, vectorBytes);

    cudaMemcpy(dA, A, vectorBytes, cudaMemcpyHostToDevice);
    cudaMemcpy(dB, B, vectorBytes, cudaMemcpyHostToDevice);

    int threadsPerBlockVec = 256;
    int blocksPerGridVec = (N + threadsPerBlockVec - 1) / threadsPerBlockVec;

    add<<<blocksPerGridVec, threadsPerBlockVec>>>(dA, dB, dC, N);
    cudaDeviceSynchronize();
    cudaMemcpy(C, dC, vectorBytes, cudaMemcpyDeviceToHost);
    ...
    int* dD, * dE, * dF;
    cudaMalloc(&dD, matrixBytes);
    cudaMalloc(&dE, matrixBytes);
    cudaMalloc(&dF, matrixBytes);

    cudaMemcpy(dD, D, matrixBytes, cudaMemcpyHostToDevice);
    cudaMemcpy(dE, E, matrixBytes, cudaMemcpyHostToDevice);

    dim3 threadsPerBlockMat(2, 2);
    dim3 blocksPerGridMat((N + 1) / 2, (N + 1) / 2);

    multiply<<<blocksPerGridMat, threadsPerBlockMat>>>(dD, dE, dF, N);
    cudaDeviceSynchronize();
    cudaMemcpy(F, dF, matrixBytes, cudaMemcpyDeviceToHost);

    printf("Multiplication: \n");
    printMatrix(F, N);

    free(D); free(E); free(F);
    cudaFree(dD); cudaFree(dE); cudaFree(dF);
